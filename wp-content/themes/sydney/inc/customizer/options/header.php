...
// Both settings are postMessage, so without a partial the preview never repaints
// the contact anchors. One partial tied to both settings avoids a redundant
// second AJAX render when both fields change together. fallback_refresh stays
// off so editing a field that is not placed in any header row does not reload
// the whole preview.
$wp_customize->selective_refresh->add_partial( 'header_contact_mail', array(
	'selector'            => '.header-contact',
	'settings'            => array( 'header_contact_mail', 'header_contact_phone' ),
	'container_inclusive' => true,
	'fallback_refresh'    => false,
	'render_callback'     => 'sydney_partial_header_contact_info',
) );
...
$wp_customize->add_setting(
	'global_search_bar_background_color',
	array(
		'default'           => '',
		'sanitize_callback' => 'wp_kses_post',
		'transport'         => 'postMessage',
...
$wp_customize->add_control(
	new Sydney_Alpha_Color(
		...
		'search_bar_background_color',
		array(
			'label'          => esc_html__( 'Search bar background color', 'sydney' ),
			'section'        => 'sydney_section_main_header',
			'settings'       => array(
				'global'  => 'global_search_bar_background_color',
				'setting' => 'search_bar_background_color',
			),
		)
	)
);
...
$wp_customize->add_control( new Sydney_Responsive_Slider( $wp_customize, 'header_icons_size',
	array(
		'label'         => esc_html__( 'Icons size', 'sydney' ),
		'section'       => 'sydney_section_main_header',
		'is_responsive' => 0,
		'settings'      => array(
			'size_desktop'      => 'header_icons_size',
		),
		'input_attrs' => array(
			'min'   => 12,
			'max'   => 24,
